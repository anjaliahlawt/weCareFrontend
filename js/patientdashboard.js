// Add this HTML for the search input box and button
...
async function fetchDoctors(specialty = '', location = '') {
    try {
        const response = await fetch(`${backendURL}doctors/all?specialty=${specialty}&location=${location}`);
        const data = await response.json();
...
function renderDoctors(doctors) {
    ...
    doctors.forEach(doctor => {
        ...
        const doctorLocation = document.createElement('l');
        doctorLocation.className = 'doctor-location';
        doctorLocation.textContent = "Location: " + doctor.location;
        doctorCard.appendChild(doctorLocation);
...
searchButton.addEventListener('click', () => {
    const specialty = searchInput.value;
    const location = locationInput.value;
    fetchDoctors(specialty, location);
});
